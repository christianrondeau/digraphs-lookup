...
{
	// Author: @Socolin
	public class MemoryArraySearchDigraphsLookupAvx : IDigraphsLookup
	{
		...
		public async Task<LookupResult[]> LookupAsync(Stream stream, params string[] digraphs)
		{
			...
			unsafe
			{
				fixed (byte* bytesPointer = new ReadOnlySpan<byte>(bytes))
				{
					...
					{
						var vector1 = Avx2.LoadVector128(i + bytesPointer);
						var vector2 = Avx2.LoadVector128(i + bytesPointer + 1);
